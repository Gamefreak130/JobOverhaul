...
using static NRaas.CareerSpace.Interactions.RegisterAsSelfEmployed.Definition;
                    ...
                    if (SavedOccupationsForTravel.ContainsKey(id))
                    {
                        if (SavedOccupationsForTravel[id] != null)
                        {
                            ...
                            if (occupation is SkillBasedCareer)
                            {
                                ...
                                {
                                    Career = new OmniCareer() { mCareerGuid = savedOccupation.Guid }
                                };
                            }
...
namespace Gamefreak130.JobOverhaulSpace.Interactions
{
    ...
    public class GetJobInRabbitHoleEx : NRaas.CareerSpace.Interactions.GetJobInRabbitHole
    {
        new public class Definition : InteractionDefinition<Sim, RabbitHole, GetJobInRabbitHoleEx>
        {
            ...
            public override string[] GetPath(bool isFemale)
            {
                return mLocation != null && mLocation.Career is Unemployed
                    ? (new string[]
                      {
                          LocalizeString(isFemale, "JoinCareer" + mLocation.Career.Guid.ToString(), new object[]
                          {
                              mLocation.Career.Name
                          })
                      })
            ...
            public override void AddInteractions(InteractionObjectPair iop, Sim actor, RabbitHole target, List<InteractionObjectPair> results)
            {
                        ...
                        foreach (CareerLocation current in target.CareerLocations.Values)
                        {
                            if (current.Career != null)
                            {
                                if (current.Career is School)
                                {
                                    ...
                                    if (school == null || (school.CareerLoc != current && school.Guid != current.Career.Guid))
                                    {
                                        string text = LocalizeString(actor.IsFemale, "JoinCareer" + current.Career.Guid, new object[]
                                        {
                                            current.Career.Name
                                        });
                                        ...
                                        results.Add(new InteractionObjectPair(new Definition(text, current), target));
                                    }
                                }
                                else if (actor.Occupation == null || (actor.Occupation.CareerLoc != current && actor.Occupation.Guid != current.Career.Guid))
                                {
                                    if (current.Career is Unemployed)
                                    {
                                        Unemployed unemployed = current.Career as Unemployed;
                                        using (List<Pair<string, string>>.Enumerator enumerator2 = unemployed.GetLocalizedTitles(actor.IsFemale).GetEnumerator())
                                        {
                                            while (enumerator2.MoveNext())
                                            {
                                                Pair<string, string> current2 = enumerator2.Current;
                                                results.Add(new InteractionObjectPair(new Definition(current2, current), target));
                                            }
                                            continue;
                                        }
                                    }
                                    string text2 = LocalizeString(actor.IsFemale, "JoinCareer" + current.Career.Guid, new object[]
                                    {
                                        current.Career.Name
                                    });
                                    ...
                                    results.Add(new InteractionObjectPair(new Definition(text2, current), target));
                                }
                ...
                {
                    NRaas.Common.Exception(actor, target, exception);
                }
            }
            ...
            public override bool Test(Sim a, RabbitHole target, bool isAutonomous, ref GreyedOutTooltipCallback greyedOutTooltipCallback)
            {
                ...
                try
                {
                    ...
                    {
                        greyedOutTooltipCallback = NRaas.Common.DebugTooltip("Boarding School");
                        result = false;
                    }
                    else if (GameUtils.GetCurrentWorldType() == WorldType.Vacation)
                    {
                        greyedOutTooltipCallback = NRaas.Common.DebugTooltip("On Vacation");
                        result = false;
                    }
                    else if (a.SimDescription.ToddlerOrBelow)
                    {
                        greyedOutTooltipCallback = NRaas.Common.DebugTooltip("ToddlerOrBelow");
                        result = false;
                    }
                    ...
                    {
                        if (mLocation.Career is SchoolElementary)
                        {
                            if (!a.SimDescription.Child)
                            {
                                greyedOutTooltipCallback = NRaas.Common.DebugTooltip("Elementary Not Child");
                                result = false;
                                return result;
                            }
                        }
                        else if (mLocation.Career is SchoolHigh)
                        {
                            ...
                            {
                                greyedOutTooltipCallback = NRaas.Common.DebugTooltip("High Not Teen");
                                result = false;
                                return result;
                            }
                        }
                        else if (mLocation.Career is School)
                        {
                            ...
                            {
                                greyedOutTooltipCallback = NRaas.Common.DebugTooltip("School Not Child Or Teen");
                                result = false;
                                return result;
                            }
                        }
                        else if (a.School != null && a.SimDescription.Teen && AfterschoolActivity.DoesJobConflictWithActivities(a, mLocation.Career))
                        {
                            greyedOutTooltipCallback = NRaas.Common.DebugTooltip("DoesJobConflictWithActivities");
                            result = false;
                            return result;
                        }
                        if (!mLocation.Career.CareerAgeTest(a.SimDescription))
                        {
                            greyedOutTooltipCallback = NRaas.Common.DebugTooltip("CareerAgeTest");
                            result = false;
                        }
                        else
                        {
                            Career occupationAsCareer = a.OccupationAsCareer;
                            Settings.mInterviewSettings.TryGetValue(mLocation.Career.SharedData.Name.Substring(34), out PersistedSettings.InterviewSettings interviewSettings);
                            Settings.mCareerAvailabilitySettings.TryGetValue(mLocation.Career.SharedData.Name.Substring(34), out PersistedSettings.CareerAvailabilitySettings availabilitySettings);
                            if (occupationAsCareer != null && occupationAsCareer.Guid == mLocation.Career.Guid)
                            {
                                if (occupationAsCareer.CareerLoc == mLocation)
                                {
                                    greyedOutTooltipCallback = NRaas.Common.DebugTooltip("Already Working There");
                                    result = false;
                                    return result;
                                }
                            }
                            if (interviewSettings == null || availabilitySettings == null || !Settings.mEnableGetJobInRabbitHole || !availabilitySettings.IsAvailable || interviewSettings.RequiresInterview || !mLocation.Career.CanAcceptCareer(a.ObjectId, ref greyedOutTooltipCallback))
                            {
                                result = false;
                                return result;
                            }
                ...
                {
                    NRaas.Common.Exception(a, target, exception);
                    result = false;
                }
                return result;
            }
        ...
        public override bool InRabbitHole()
        {
            ...
            try
            {
                ...
                {
                    if (Actor.Occupation is Unemployed unemployed)
                    {
                        unemployed.UpdateName(definition.mName.Second);
                    }
                    result = true;
                }
            }
            ...
            {
                if (Actor.CareerManager.mJob == null)
                {
        ...
        public class RegisterAsSelfEmployedCityHall : CityHall.RegisterAsSelfEmployed
        {
            ...
            public override bool InRabbitHole()
            {
                ...
                if (flag2)
                {
                    ...
                    Career career = new OmniCareer
                    {
                        mCareerGuid = CareerToSet
                    };
                    occupationParameters.Location.Career = career;
